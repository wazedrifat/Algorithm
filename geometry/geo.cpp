...
bool PointOnSegment(PT s, PT e, PT p)
{
	if (p == s || p == e)
		return 1;
	return dcmp(cross(s - p, s - e)) == 0 && dcmp(dot(PT(s.x - p.x, s.y - p.y), PT(e.x - p.x, e.y - p.y))) < 0;
}
...
{ //The area is positive if abc is in counter-clockwise direction
...
void MakeConvexHull(vector<PT> given, vector<PT> &ans)
{
	...
	for (i = 0; i < n; i++)
	{
		while (true)
		{
			if (j < 2)
				break;
			if (SignedArea(L[j - 2], L[j - 1], given[i]) <= EPS)
				j--;
			else
				break;
		}
	...
	for (i = n - 1; i >= 0; i--)
	{
		while (1)
		{
			if (k < 2)
				break;
			if (SignedArea(U[k - 2], U[k - 1], given[i]) <= EPS)
				k--;
			else
				break;
		}
