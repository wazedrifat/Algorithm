    ...
    {
        visited[u]=true;
        for (int i=0; i<graph[u].sz; i++) if (!visited[graph[u][i]]) pre_dfs(graph[u][i]);
        S.push(u);
    }
    void pre_dfs2(int u, int p)
    {
        visited[u]=true;
        P[u]=p;
        for (int i=0; i<trans[u].sz; i++) if (!visited[trans[u][i]]) pre_dfs2(trans[u][i], p);
    }
    int convert_to_DAG()
    {
        int i, j, cnt=0;
        MEM(visited, 0);
        for (i=1; i<=n; i++) if (!visited[i]) pre_dfs(i);
